...
static const char paritymode_table[] = {
   [XPT_UART_PARITY_NONE] = 'N',
   [XPT_UART_PARITY_EVEN] = 'E',
   [XPT_UART_PARITY_ODD] = 'O',
   [XPT_UART_PARITY_MARK] = 'M',
   [XPT_UART_PARITY_SPACE] = 'S'
};
...
uarttool_find(const char *str) {
    ...
    int uart_index = xpt_uart_lookup(str);
    if (uart_index < 0) {
        ...
        if (isdigit(*str)) {
            ...
            const char *devpath;
            int number_of_uarts = xpt_get_uart_count();
            for (uart_index = 0; uart_index < number_of_uarts; uart_index++) {
                 xpt_result_t res = xpt_uart_settings(uart_index, NULL, &devpath, NULL, NULL, NULL, NULL, NULL, NULL);
                 if (res == XPT_SUCCESS && devpath != NULL && !strcmp(devpath, str)) break;
            }
            if (uart_index >= number_of_uarts) {
                 return xpt_uart_init_raw(str);
            }
        }
    }

    return xpt_uart_init(uart_index);
}
...
void
uarttool_read_and_print(xpt_uart_context uart, double receive_timeout) {
   char buf;
   do {
       double start = now();
       if (xpt_uart_data_available(uart, 1.0 + receive_timeout * 1000.0)) {
           xpt_uart_read(uart, &buf, 1);
           putchar(buf);
           fflush(stdout);
       }
...
main(int argc, const char** argv) {
    xpt_uart_context uart = NULL;
    int uart_index, i;

    int baudrate = 115200, stopbits = 1, databits = 8;
    xpt_uart_parity_t parity = XPT_UART_PARITY_NONE;
    ...
    /* Initialize XPT. Init is done automatically if libxpt is compiled
    ...
    */
    ...
    if (!strcmp(argv[1], "list")) {
        int number_of_uarts = xpt_get_uart_count();
        for (uart_index = 0; uart_index < number_of_uarts; uart_index++) {
             xpt_result_t res = xpt_uart_settings(uart_index, &name, &dev, &baudrate, &databits, &stopbits, &parity, &ctsrts, &xonxoff);
             if (res == XPT_SUCCESS) {
        ...
        for (i=1; i<argc; i++) {
            if (!strcmp(argv[i], "dev")) {
                ...
                dev = xpt_uart_get_dev_path(uart);
                xpt_result_t res = xpt_uart_settings(-1, &dev, &name, &baudrate, &databits, &stopbits, &parity, &ctsrts, &xonxoff);
                if (res != XPT_SUCCESS) {
            ...
            if (!strcmp(argv[i], "baud") || !strcmp(argv[i], "baudrate")) {
                ...
                if (uart != NULL) {
                    xpt_uart_set_baudrate(uart, baudrate);
                }
                i++;
            } else
            ...
            if (!strcmp(argv[i], "ctsrts") || !strcmp(argv[i], "rtscts") || !strcmp(argv[i], "crtscts")) {
                ...
                xonxoff = 0u;
                if (uart != NULL) {
                    xpt_uart_set_flowcontrol(uart, xonxoff, ctsrts);
                }
                i++;
            } else
            ...
            if (!strcmp(argv[i], "stopbits")) {
                ...
                if (uart != NULL) {
                    xpt_uart_set_mode(uart, databits, parity, stopbits);
                }
                i++;
            } else
        ...
        if (i == argc && uart != NULL) {
            if (send) {
                xpt_uart_write(uart, to_send, strlen(to_send)+1);
            }
            ...
            if (show) {
                if (recieve) putchar('\n');
                dev = xpt_uart_get_dev_path(uart);
                xpt_uart_settings(-1, &dev, &name, &baudrate, &databits, &stopbits, &parity, &ctsrts, &xonxoff);
