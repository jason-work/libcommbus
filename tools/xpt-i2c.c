...
print_version()
{
    fprintf(stdout, "Version %s on %s", xpt_get_version(), xpt_get_platform_name());
...
void
print_bus(xpt_board_t* board)
{
    int i, bus;
    for (i = 0; i < board->i2c_bus_count; ++i) {
        char* busType;
        switch (board->platform_type) {
            case XPT_FTDI_FT4222:
                busType = "ft4222";
                bus = xpt_get_sub_platform_id(i);
                break;
            default:
                busType = "linux";
                bus = i;
...
i2c_get(int bus, uint8_t device_address, uint8_t register_address, uint8_t* data)
{
    ...
    xpt_i2c_context i2c = xpt_i2c_init(bus);
    if (i2c == NULL) {
        return XPT_ERROR_NO_RESOURCES;
    }
    status = xpt_i2c_address(i2c, device_address);
    if (status != XPT_SUCCESS) {
        goto i2c_get_exit;
    }
    status = xpt_i2c_write_byte(i2c, register_address);
    if (status != XPT_SUCCESS) {
        goto i2c_get_exit;
    }
    status = xpt_i2c_read(i2c, data, 1) == 1 ? XPT_SUCCESS : XPT_ERROR_UNSPECIFIED;
    if (status != XPT_SUCCESS) {
        goto i2c_get_exit;
    }
i2c_get_exit:
...
i2c_set(int bus, uint8_t device_address, uint8_t register_address, uint8_t data)
{
    ...
    status = xpt_i2c_write_byte_data(i2c, data, register_address);
    if (status != XPT_SUCCESS) {
...
i2c_detect_devices(int bus)
{
    ...
    for (addr = 0x0; addr < 0x80; ++addr) {
        ...
        if ((addr) % 16 == 0)
            printf("%02x: ", addr);
        if (i2c_get(bus, addr, 0, &value) == XPT_SUCCESS)
            printf("%02x ", addr);
        else
            printf("-- ");
        if ((addr + 1) % 16 == 0)
...
process_command(int argc, char** argv)
{
    ...
    if (strncmp(argv[1], "help", strlen("help") + 1) == 0) {
        ...
        if (argc == 5) {
            int interation = 0;
            xpt_boolean_t should_repeat = strncmp(argv[1], "getrpt", strlen("getrpt") + 1) == 0;
            int bus = strtol(argv[2], NULL, 0);
            ...
            uint8_t data;
            do {
                if (i2c_get(bus, device_address, register_address, &data) == XPT_SUCCESS) {
                    if (should_repeat)
        ...
        if (argc == 6) {
            ...
            if (i2c_set(bus, device_address, register_address, value) != XPT_SUCCESS) {
