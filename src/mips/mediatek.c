...
static xpt_result_t
mtk_mmap_write(xpt_gpio_context dev, int value)
{
    if (value) {
...
static int
mtk_mmap_read(xpt_gpio_context dev)
{
...
static xpt_result_t
mtk_mmap_setup(xpt_gpio_context dev, xpt_boolean_t en)
{
    if (dev == NULL) {
...
mtk_select_function(unsigned int id, char *name)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        syslog(LOG_INFO, "xpt: set pinmux %s -> %s\n", mt7688_mux[id].name, name);
        return;
    }
}
...
static xpt_result_t
mtk_pwm_init_post(xpt_pwm_context pwm)
{
    switch(pwm->pin) {
    case 0:
...
static xpt_result_t
mtk_i2c_freq(xpt_i2c_context dev, xpt_i2c_mode_t mode)
{
    switch (mode) {
    case XPT_I2C_STD:
        break;
 ...
 */
static void
mtk_add_pin(xpt_board_t *b, int index, int pin, const char *name, int mux,
            xpt_boolean_t valid, xpt_boolean_t gpio, xpt_boolean_t pwm,
            xpt_boolean_t fast_gpio, xpt_boolean_t spi, xpt_boolean_t i2c,
            xpt_boolean_t aio, xpt_boolean_t uart)
{
    strncpy(b->pins[index].name, name, XPT_PIN_NAME_SIZE);
 ...
 */
static void
mtk_add_uart(xpt_board_t *b, char *path, int tx_pin, int rx_pin)
{
...
mtk_common(char *name, int pin_count)
{
    ...
    xpt_board_t *b = (xpt_board_t*) malloc(sizeof(xpt_board_t));
    if (b == NULL) {
        return NULL;
    }

    memset(b, 0, sizeof(xpt_board_t));
    ...
    b->pins = (xpt_pininfo_t*) malloc(sizeof(xpt_pininfo_t) * b->phy_pin_count);

    memset(b->pins, 0, sizeof(xpt_pininfo_t) * b->phy_pin_count);
    memset(gpio_mux_groups, -1, sizeof(gpio_mux_groups));

    b->adv_func->gpio_mmap_setup = &mtk_mmap_setup;

    for (i = 0; i < b->phy_pin_count; i++) {
        snprintf(b->pins[i].name, XPT_PIN_NAME_SIZE, "GPIO%d", i);
        b->pins[i].capabilities = (xpt_pincapabilities_t){ 0, 0, 0, 0, 0, 0, 0, 0 };
    }
...
xpt_mtk_linkit()
{
    xpt_board_t *b = mtk_common("LinkIt Smart 7688", 32);
    if (b == NULL) {
        return NULL;
    }
...
xpt_mtk_omega2()
{
    xpt_board_t *b = mtk_common("Onion Omega2", 32);
    if (b == NULL) {
        return NULL;
    }
