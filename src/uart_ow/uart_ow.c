...
// low-level read byte
static xpt_result_t _ow_read_byte(xpt_uart_ow_context dev, uint8_t *ch)
{
    ...
    int rv;
    do {
        rv = xpt_uart_read(dev->uart, (char*) ch, 1);
    } while (rv == 0 && (time(NULL) < thetime));

    if (rv == 0) {
        return XPT_ERROR_NO_DATA_AVAILABLE; // we timed out
    }
    else {
        return XPT_SUCCESS;
    }
...
static int _ow_write_byte(xpt_uart_ow_context dev, const char ch)
{
    return xpt_uart_write(dev->uart, &ch, 1);
}
...
//
static xpt_result_t _ow_set_speed(xpt_uart_ow_context dev, xpt_boolean_t speed)
{
...
//
static xpt_boolean_t _ow_search(xpt_uart_ow_context dev)
{
    int id_bit_number;
    ...
    if (!dev->LastDeviceFlag) {
        // 1-Wire reset
        if (xpt_uart_ow_reset(dev) != XPT_SUCCESS) {
            // reset the search
        ...
        xpt_uart_ow_write_byte(dev, XPT_UART_OW_CMD_SEARCH_ROM);

        // loop to do the search
        do {
            // read a bit and its complement
            id_bit = xpt_uart_ow_bit(dev, 1);
            cmp_id_bit = xpt_uart_ow_bit(dev, 1);
            ...
            if ((id_bit == 1) && (cmp_id_bit == 1))
                ...
                xpt_uart_ow_bit(dev, search_direction);
...
//        0 : no device present
//
static xpt_boolean_t _ow_first(xpt_uart_ow_context dev)
{
...
//         0 : device not found, end of search
//
static xpt_boolean_t _ow_next(xpt_uart_ow_context dev)
{
    // leave the search state alone
    return _ow_search(dev);
}

// Start of exported xpt functionality

xpt_uart_ow_context xpt_uart_ow_init(int index)
{
    xpt_uart_ow_context dev = calloc(1, sizeof(struct _xpt_uart_ow));
    if (!dev)
        return NULL;

    dev->uart = xpt_uart_init(index);
    if (!dev->uart)
        {
            free(dev);
            return NULL;
...
xpt_uart_ow_context xpt_uart_ow_init_raw(const char* path)
{
    ...
    dev->uart = xpt_uart_init_raw(path);
    if (!dev->uart)
        {
            free(dev);
            return NULL;
...
{
    xpt_result_t rv =  xpt_uart_stop(dev->uart);
    free(dev);
    return rv;
}
...
{
    return xpt_uart_get_dev_path(dev->uart);
}
...
int xpt_uart_ow_bit(xpt_uart_ow_context dev, uint8_t bit)
{
     ...
     */
    if (_ow_read_byte(dev, &ch) != XPT_SUCCESS || ret == -1) {
         return -1;
    }
    return (ch == 0xff);
}
...
int xpt_uart_ow_write_byte(xpt_uart_ow_context dev, uint8_t byte)
{
    ...
    int i;
    for (i = 0; i < 8; i++) {
        bit = xpt_uart_ow_bit(dev, byte & 0x01);
...
int xpt_uart_ow_read_byte(xpt_uart_ow_context dev)
{
     ...
     */
    return xpt_uart_ow_write_byte(dev, 0xff);
}
...
xpt_result_t xpt_uart_ow_reset(xpt_uart_ow_context dev)
{
     ...
     */
    if (_ow_set_speed(dev, 0) != XPT_SUCCESS) {
        return XPT_ERROR_INVALID_HANDLE;
    }
    ...
    if (_ow_read_byte(dev, &rv) != XPT_SUCCESS) {
        return XPT_ERROR_NO_DATA_AVAILABLE;
    }

    /* back up to high speed for normal data transmissions */
    if (_ow_set_speed(dev, 1) != XPT_SUCCESS) {
        return XPT_ERROR_INVALID_HANDLE;
    }

    /* shorted data line */
    if (rv == 0x00)
        return XPT_ERROR_UART_OW_SHORTED;

    /* no devices detected (no presence pulse) */
    if (rv == 0xf0)
        return XPT_ERROR_UART_OW_NO_DEVICES;
...
xpt_result_t xpt_uart_ow_rom_search(xpt_uart_ow_context dev, xpt_boolean_t start, uint8_t* id)
{
    ...
    xpt_result_t rv;
    if ((rv = xpt_uart_ow_reset(dev)) != XPT_SUCCESS)
        return rv;

    xpt_boolean_t result;
    ...
    if (result) {
        ...
        int i;
        for (i = 0; i < XPT_UART_OW_ROMCODE_SIZE; i++)
            id[i] = dev->ROM_NO[i];
...
xpt_result_t xpt_uart_ow_command(xpt_uart_ow_context dev, uint8_t command, uint8_t* id)
{
    ...
    xpt_result_t rv = xpt_uart_ow_reset(dev);
    ...
    if (id) {
        ...
        xpt_uart_ow_write_byte(dev, XPT_UART_OW_CMD_MATCH_ROM);

        /* sending to a specific device, so send out the full romcode */
        int i;
        for (i = 0; i < XPT_UART_OW_ROMCODE_SIZE; i++)
            xpt_uart_ow_write_byte(dev, id[i]);
    } else {
        /* send to all devices (or a single device if it's the only one
         * on the bus)
         */
        xpt_uart_ow_write_byte(dev, XPT_UART_OW_CMD_SKIP_ROM);
    }

    xpt_uart_ow_write_byte(dev, command);
