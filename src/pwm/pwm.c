...
static int
xpt_pwm_setup_duty_fp(xpt_pwm_context dev)
{
    char bu[MAX_SIZE];
...
static xpt_result_t
xpt_pwm_write_period(xpt_pwm_context dev, int period)
{
    if (!dev) {
    ...
    if (IS_FUNC_DEFINED(dev, pwm_period_replace)) {
        xpt_result_t result = dev->advance_func->pwm_period_replace(dev, period);
...
static xpt_result_t
xpt_pwm_write_duty(xpt_pwm_context dev, int duty)
{
    if (!dev) {
    ...
    if (dev->duty_fp == -1) {
        if (xpt_pwm_setup_duty_fp(dev) == 1) {
...
static int
xpt_pwm_read_period(xpt_pwm_context dev)
{
    if (!dev) {
...
static int
xpt_pwm_read_duty(xpt_pwm_context dev)
{
    if (!dev) {
...
xpt_pwm_init_internal(xpt_adv_func_t* func_table, int chipin, int pin)
{
...
xpt_pwm_init(int pin)
{
    ...
    if (board->adv_func->pwm_init_pre != NULL) {
        if (board->adv_func->pwm_init_pre(pin) != XPT_SUCCESS)
            return NULL;
    }

    if (board->pins[pin].pwm.mux_total > 0) {
        if (xpt_setup_mux_mapped(board->pins[pin].pwm) != XPT_SUCCESS) {
    ...
    if (board->adv_func->pwm_init_post != NULL) {
        xpt_pwm_context pret = xpt_pwm_init_raw(chip, pinn);
        xpt_result_t ret = board->adv_func->pwm_init_post(pret);
        if (ret != XPT_SUCCESS) {
            free(pret);
            return NULL;
        }
    ...
    return xpt_pwm_init_raw(chip, pin);
#else
    return xpt_pwm_init_raw(chip, pinn);
#endif
}

xpt_pwm_context
xpt_pwm_init_raw(int chipin, int pin)
{
    xpt_pwm_context dev = xpt_pwm_init_internal(plat == NULL ? NULL : plat->adv_func , chipin, pin);
    if (dev == NULL) {
    ...
    if (IS_FUNC_DEFINED(dev, pwm_init_raw_replace)) {
        if (dev->advance_func->pwm_init_raw_replace(dev, pin) == XPT_SUCCESS) {
            return dev;
        } else {
            free(dev);
            return NULL;
    ...
    if (stat(directory, &dir) == 0 && S_ISDIR(dir.st_mode)) {
        ...
        dev->owner = 1;
        xpt_pwm_period_us(dev, plat->pwm_default_period);
        close(export_f);
    }

    xpt_pwm_setup_duty_fp(dev);

    return dev;
}

xpt_result_t
xpt_pwm_write(xpt_pwm_context dev, float percentage)
{
    if (!dev) {
    ...
    if (IS_FUNC_DEFINED(dev, pwm_write_pre)) {
        if (dev->advance_func->pwm_write_pre(dev, percentage) != XPT_SUCCESS) {
            syslog(LOG_ERR, "xpt_pwm_write (pwm%i): pwm_write_pre failed, see syslog", dev->pin);
            return XPT_ERROR_UNSPECIFIED;
        }
    }

    if (dev->period == -1) {
        if (xpt_pwm_read_period(dev) <= 0)
            return XPT_ERROR_NO_DATA_AVAILABLE;
    }

    if (percentage > 1.0f) {
        syslog(LOG_WARNING, "pwm_write: %i%% entered, defaulting to 100%%",(int) percentage * 100);
        return xpt_pwm_write_duty(dev, dev->period);
    }
    return xpt_pwm_write_duty(dev, percentage * dev->period);
}

float
xpt_pwm_read(xpt_pwm_context dev)
{
    if (!dev) {
    ...
    int period = xpt_pwm_read_period(dev);
    if (period > 0) {
        return (xpt_pwm_read_duty(dev) / (float) period);
    }
    return 0.0f;
}

xpt_result_t
xpt_pwm_period(xpt_pwm_context dev, float seconds)
{
    return xpt_pwm_period_ms(dev, seconds * 1000);
}

xpt_result_t
xpt_pwm_period_ms(xpt_pwm_context dev, int ms)
{
    return xpt_pwm_period_us(dev, ms * 1000);
}

xpt_result_t
xpt_pwm_period_us(xpt_pwm_context dev, int us)
{
    int min, max;
    ...
    if (xpt_is_sub_platform_id(dev->chipid)) {
    ...
    return xpt_pwm_write_period(dev, us * 1000);
}

xpt_result_t
xpt_pwm_pulsewidth(xpt_pwm_context dev, float seconds)
{
    return xpt_pwm_pulsewidth_ms(dev, seconds * 1000);
}

xpt_result_t
xpt_pwm_pulsewidth_ms(xpt_pwm_context dev, int ms)
{
    return xpt_pwm_pulsewidth_us(dev, ms * 1000);
}

xpt_result_t
xpt_pwm_pulsewidth_us(xpt_pwm_context dev, int us)
{
    return xpt_pwm_write_duty(dev, us * 1000);
}

xpt_result_t
xpt_pwm_enable(xpt_pwm_context dev, int enable)
{
    if (!dev) {
    ...
    if (IS_FUNC_DEFINED(dev, pwm_enable_pre)) {
        if (dev->advance_func->pwm_enable_pre(dev, enable) != XPT_SUCCESS) {
            syslog(LOG_ERR, "xpt_pwm_enable (pwm%i): pwm_enable_pre failed, see syslog", dev->pin);
            return XPT_ERROR_UNSPECIFIED;
        }
    }
...
xpt_result_t
xpt_pwm_unexport_force(xpt_pwm_context dev)
{
...
xpt_result_t
xpt_pwm_unexport(xpt_pwm_context dev)
{
    if (!dev) {
    ...
    xpt_pwm_enable(dev, 0);
    if (dev->owner) {
        return xpt_pwm_unexport_force(dev);
    }
    return XPT_ERROR_INVALID_PARAMETER;
}

xpt_result_t
xpt_pwm_close(xpt_pwm_context dev)
{
    if (!dev) {
...
xpt_result_t
xpt_pwm_owner(xpt_pwm_context dev, xpt_boolean_t owner_new)
{
    if (!dev) {
...
int
xpt_pwm_get_max_period(xpt_pwm_context dev)
{
    if (plat == NULL) {
        return -1;
    }
...
int
xpt_pwm_get_min_period(xpt_pwm_context dev)
{
    if (plat == NULL) {
        return -1;
    }
