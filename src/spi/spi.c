...
static xpt_spi_context xpt_spi_init_internal(xpt_adv_func_t* func_table)
{
    xpt_spi_context dev = (xpt_spi_context) calloc(1, sizeof(struct _spi));
    if (dev == NULL) {
        return NULL;
    }
...
xpt_spi_context xpt_spi_init(int bus)
{
    if (plat == NULL) {
    ...
    if (plat->adv_func != NULL && plat->adv_func->spi_init_pre != NULL) {
        if (plat->adv_func->spi_init_pre(bus) != XPT_SUCCESS) {
            return NULL;
        }
    }
    ...
    if (!plat->no_bus_mux) {
        ...
        if (pos >= 0 && plat->pins[pos].spi.mux_total > 0) {
            if (xpt_setup_mux_mapped(plat->pins[pos].spi) != XPT_SUCCESS) {
    ...
    xpt_spi_context dev = xpt_spi_init_raw(plat->spi_bus[bus].bus_id, plat->spi_bus[bus].slave_s);

    if (plat->adv_func != NULL && plat->adv_func->spi_init_post != NULL) {
        xpt_result_t ret = plat->adv_func->spi_init_post(dev);
        if (ret != XPT_SUCCESS) {
            free(dev);
            return NULL;
        }
...
xpt_spi_context xpt_spi_init_raw(unsigned int bus, unsigned int cs)
{
    xpt_result_t status = XPT_SUCCESS;

    xpt_spi_context dev = xpt_spi_init_internal(plat == NULL ? NULL : plat->adv_func);
    if (dev == NULL) {
    ...
    status = xpt_spi_mode(dev, XPT_SPI_MODE0);
    if (status != XPT_SUCCESS) {
        goto init_raw_cleanup;
    }

    status = xpt_spi_lsbmode(dev, 0);
    if (status != XPT_SUCCESS) {
        goto init_raw_cleanup;
    }

    status = xpt_spi_bit_per_word(dev, 8);
    if (status != XPT_SUCCESS) {
        goto init_raw_cleanup;
    }
...
xpt_result_t xpt_spi_mode(xpt_spi_context dev, xpt_spi_mode_t mode)
{
    if (dev == NULL) {
...
xpt_result_t xpt_spi_frequency(xpt_spi_context dev, int hz)
{
    if (dev == NULL) {
...
xpt_result_t xpt_spi_lsbmode(xpt_spi_context dev, xpt_boolean_t lsb)
{
    if (dev == NULL) {
...
xpt_result_t xpt_spi_bit_per_word(xpt_spi_context dev, unsigned int bits)
{
    if (dev == NULL) {
...
int xpt_spi_write(xpt_spi_context dev, uint8_t data)
{
    if (dev == NULL) {
...
int xpt_spi_write_word(xpt_spi_context dev, uint16_t data)
{
    if (dev == NULL) {
...
xpt_result_t xpt_spi_transfer_buf(xpt_spi_context dev, uint8_t* data, uint8_t* rxbuf, int length)
{
    if (dev == NULL) {
...
xpt_result_t xpt_spi_transfer_buf_word(xpt_spi_context dev, uint16_t* data, uint16_t* rxbuf, int length)
{
    if (dev == NULL) {
...
uint8_t* xpt_spi_write_buf(xpt_spi_context dev, uint8_t* data, int length)
{
    if (dev == NULL) {
    ...
    if (xpt_spi_transfer_buf(dev, data, recv, length) != XPT_SUCCESS) {
        free(recv);
        return NULL;
    }
    return recv;
...
uint16_t* xpt_spi_write_buf_word(xpt_spi_context dev, uint16_t* data, int length)
{
    if (dev == NULL) {
    ...
    if (xpt_spi_transfer_buf_word(dev, data, recv, length) != XPT_SUCCESS) {
        free(recv);
        return NULL;
    }
    return recv;
...
xpt_result_t xpt_spi_stop(xpt_spi_context dev)
{
    if (dev == NULL) {
