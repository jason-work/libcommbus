...
static xpt_uart_context xpt_uart_init_internal(xpt_adv_func_t* func_table)
{
    xpt_uart_context dev = (xpt_uart_context) calloc(1, sizeof(struct _uart));
    if (dev == NULL) {
...
xpt_uart_context xpt_uart_init(int index)
{
    if (plat == NULL) {
    ...
    if (xpt_is_sub_platform_id(index)) {
    ...
    if (plat->adv_func != NULL && plat->adv_func->uart_init_pre != NULL) {
        if (plat->adv_func->uart_init_pre(index) != XPT_SUCCESS) {
    ...
    if (!plat->no_bus_mux) {
        ...
        if (pos >= 0) {
            if (plat->pins[pos].uart.mux_total > 0) {
                if (xpt_setup_mux_mapped(plat->pins[pos].uart) != XPT_SUCCESS) {
    ...
    xpt_uart_context dev = xpt_uart_init_raw((char*)plat->uart_dev[index].device_path);
    if (dev == NULL) {
        return NULL;
    }
    ...
    if (IS_FUNC_DEFINED(dev, uart_init_post)) {
        xpt_result_t ret = dev->advance_func->uart_init_post(dev);
        if (ret != XPT_SUCCESS) {
            free(dev);
            return NULL;
        }
...
xpt_uart_context xpt_uart_init_raw(const char* path)
{
    xpt_result_t status = XPT_SUCCESS;
    xpt_uart_context dev = NULL;

    if (!path) {
        syslog(LOG_ERR, "uart: device path undefined");
        status = XPT_ERROR_INVALID_PARAMETER;
        goto init_raw_cleanup;
    }

    dev = xpt_uart_init_internal(plat == NULL ? NULL : plat->adv_func);
    if (dev == NULL) {
        syslog(LOG_ERR, "uart: Failed to allocate memory for context");
        status = XPT_ERROR_NO_RESOURCES;
        goto init_raw_cleanup;
    }
    ...
    if (xpt_uart_set_baudrate(dev, 9600) != XPT_SUCCESS) {
        status = XPT_ERROR_INVALID_RESOURCE;
        goto init_raw_cleanup;
    }
...
xpt_result_t xpt_uart_stop(xpt_uart_context dev)
{
    if (!dev) {
...
xpt_result_t xpt_uart_settings(int index, const char **devpath, const char **name, int* baudrate, int* databits, int* stopbits, xpt_uart_parity_t* parity, unsigned int* ctsrts, unsigned int* xonxoff) {
    struct termios term;
    int fd;
    ...
    if (baudrate != NULL || stopbits != NULL || parity != NULL || databits != NULL || ctsrts != NULL || xonxoff != NULL) {
       ...
       if (parity != NULL) {
           if (term.c_cflag & PARODD) *parity = XPT_UART_PARITY_ODD;
           else
           if (term.c_cflag & PARENB) *parity = XPT_UART_PARITY_EVEN;
           else
           *parity = XPT_UART_PARITY_NONE;
       }
...
xpt_result_t xpt_uart_flush(xpt_uart_context dev)
{
    if (!dev) {
...
xpt_result_t xpt_uart_sendbreak(xpt_uart_context dev, int duration)
{
    if (!dev) {
...
xpt_result_t xpt_uart_set_baudrate(xpt_uart_context dev, unsigned int baud)
{
    if (!dev) {
...
xpt_result_t xpt_uart_set_mode(xpt_uart_context dev, int bytesize, xpt_uart_parity_t parity, int stopbits)
{
    if (!dev) {
    ...
    switch (parity) {
        case XPT_UART_PARITY_NONE:
            termio.c_cflag &= ~(PARENB | PARODD);
            break;
        case XPT_UART_PARITY_EVEN:
            termio.c_cflag |= PARENB;
            termio.c_cflag &= ~PARODD;
            break;
        case XPT_UART_PARITY_ODD:
            termio.c_cflag |= PARENB | PARODD;
            break;
        case XPT_UART_PARITY_MARK: // not POSIX
            termio.c_cflag |= PARENB | CMSPAR | PARODD;
            break;
        case XPT_UART_PARITY_SPACE: // not POSIX
...
xpt_result_t xpt_uart_set_flowcontrol(xpt_uart_context dev, xpt_boolean_t xonxoff, xpt_boolean_t rtscts)
{
    if (!dev) {
        ...
        if (!plat->no_bus_mux) {
            ...
            if ((pos_cts >= 0) && (pos_rts >= 0)) {
                if (plat->pins[pos_cts].uart.mux_total > 0) {
                    if (xpt_setup_mux_mapped(plat->pins[pos_cts].uart) != XPT_SUCCESS) {
                        syslog(LOG_ERR, "uart%i: init: failed to setup muxes for CTS pin", dev->index);
                        return XPT_ERROR_FEATURE_NOT_SUPPORTED;
                    }
                }
                if (plat->pins[pos_rts].uart.mux_total > 0) {
                    if (xpt_setup_mux_mapped(plat->pins[pos_rts].uart) != XPT_SUCCESS) {
...
xpt_result_t xpt_uart_set_timeout(xpt_uart_context dev, int read, int write, int interchar)
{
    if (!dev) {
...
xpt_result_t xpt_uart_set_non_blocking(xpt_uart_context dev, xpt_boolean_t nonblock)
{
    if (!dev) {
...
const char* xpt_uart_get_dev_path(xpt_uart_context dev)
{
    if (!dev) {
        return NULL;
    }
...
int xpt_uart_read(xpt_uart_context dev, char* buf, size_t len)
{
    if (!dev) {
...
int xpt_uart_write(xpt_uart_context dev, const char* buf, size_t len)
{
    if (!dev) {
...
xpt_boolean_t xpt_uart_data_available(xpt_uart_context dev, unsigned int millis)
{
    if (!dev) {
