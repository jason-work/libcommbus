 ...
 * libxpt. It allows the exposure of UART pins on supported boards.
...
/** Xpt Uart Context */
typedef struct _uart* xpt_uart_context;
 ...
 */
xpt_uart_context xpt_uart_init(int uart);
 ...
 * "xpt_uart_set_mode(dev, 8,XPT_UART_PARITY_NONE , 1)"
 *
 * @param dev The UART context
 ...
 */
xpt_result_t
xpt_uart_settings(int index,
    const char **devpath,
    const char **name,
    int* baudrate,
    int* databits,
    int* stopbits,
    xpt_uart_parity_t* parity,
    unsigned int* rtscts,
    unsigned int* xonxoff);

/**
 * Destroy a xpt_uart_context
 *
 * @param dev uart context
 * @return xpt_result_t
 */
 ...
 */
int xpt_uart_read(xpt_uart_context dev, char* buf, size_t length);
 ...
 */
int xpt_uart_write(xpt_uart_context dev, const char* buf, size_t length);
